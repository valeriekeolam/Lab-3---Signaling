...
void report(int signal_number){
	...
	while(1){
		if(miniCounter < 10){ //Increments the miniCounter (for when 10 signals reached)
			if(signal_number == SIGUSR1){
				miniCounter++;
			} else if (signal_number == SIGUSR2){
				miniCounter++;
			...
			printf("Avg Time between Receptions: %ld\n", shm_ptr->sig_gotUSRone/shm_ptr->timesum_sigone);

			printf("\nSIGUSR2 Sent: %d\n", shm_ptr->sig_sentUSRtwo);
			printf("SIGUSR2 Received: %d\n", shm_ptr->sig_gotUSRtwo);
			printf("Lost SIGUSR2 signals: %d\n", sig_two_lost);
			printf("Avg Time between Receptions: %ld", shm_ptr->sig_gotUSRtwo/shm_ptr->timesum_sigtwo);
		}
	}
